module Calypso.Frontend.InScope
  ( InScope
  , forRuntime
  ) where
...
browserScope =
  ...
  , autoImports:
      ...
      , "Calypso.Runtime (toCalypsoValue, emit, done)"
      ]
  , highlightedPackages:
  ...
  , notes:
      ...
      , "Return `Calypso.Runtime.ForceRender {...}` or a string starting with `<svg` to have the value drawn in the Render column."
      ]
  }
