module Calypso.Server.Synthesize
  ( synthesize
  , Synthesised
  ) where
...
-- | what `Calypso.User` re-exports (which is only its *declarations*,
...
-- | list) with `module Calypso.User where`. If no module header is
...
rewriteModuleHeader src =
  case Str.indexOf (Pattern "module") src of
    Nothing -> "module Calypso.User where\n" <> src
    Just moduleIdx ->
      ...
      in case Str.indexOf (Pattern "where") afterModule of
        ...
        Just relWhereIdx ->
          ...
          in before <> "module Calypso.User where" <> after
...
buildMain opts userImports cells =
  let
    ...
    -- Purerl's ToCalypsoValue returns `Effect` directly (no Aff
    -- in the Purerl set), so both sides sit in Effect naturally.
    emitLine (Cell c) =
      if opts.purerl
        then
          "  v_" <> c.id <> " <- toCalypsoValue cell_" <> c.id
            <> "\n  emit \"" <> c.id <> "\" v_" <> c.id <> "\n"
        else
          "  v_" <> c.id <> " <- toCalypsoValue cell_" <> c.id
