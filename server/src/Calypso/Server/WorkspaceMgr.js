...
// inside the outer workspace. Main.purs and Calypso/User.purs are
...
const TEMPLATE_COPY_PATHS = [
  'src/Calypso/Runtime.purs',
  'src/Calypso/Runtime.js',
];

const REWRITE_PATHS = ['spago.yaml', 'package.json'];

const SEED_MAIN_PURS = 'module Main where\n';
const SEED_USER_PURS = 'module Calypso.User where\n\nimport Prelude\n';
...
async function createWorkspace(rootDir, templateDir, packageName, id) {
  ...
  await mkdir(pathJoin(workspaceDir, 'src', 'Calypso'), { recursive: true });
  ...
  await Promise.all([
    ...
    writeFile(
      pathJoin(workspaceDir, 'src', 'Calypso', 'User.purs'),
      SEED_USER_PURS,
      'utf8',
    ),
  ]);
...
export const _createWorkspaceDirSync =
  (rootDir) => (templateDir) => (packageName) => (id) => () => {
    const workspaceDir = pathJoin(rootDir, id);
    mkdirSync(pathJoin(workspaceDir, 'src', 'Calypso'), { recursive: true });
    for (const rel of TEMPLATE_COPY_PATHS) {
